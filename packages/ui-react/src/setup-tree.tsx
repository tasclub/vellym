...
export function SetupTree(props: {
      ...
      return (
        <li key={node.id} role="none">
          <div
            ...
            className={`setup-tree-item ${node.kind}${props.readOnly ? " readonly" : ""}${omitted ? " omitted" : ""}`}
          ...
          >
            {expandable ? (
              ...
              <span className="setup-tree-twisty" aria-hidden="true" />
            )}
            ...
            <span className="setup-tree-label">
              <span className="setup-tree-title">
                <strong>{node.title}</strong>
                {omitted && (
                  <span className="setup-tree-omitted">
                    {ja ? "作成しません" : "Not created"}
                  </span>
                )}
              </span>
              <small className="setup-tree-description">{node.description}</small>
              {(node.referenceModels.length > 0 || reason) && (
                <small id={reason ? `${node.id}-reason` : undefined} className="setup-tree-meta">
  ...
  return (
    <ul role="tree" aria-label={props.label} className="setup-tree">
