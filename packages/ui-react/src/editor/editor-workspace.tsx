...
export function EditorWorkspace(props: {
  /**
   * 見出しと本文の間に差し込む内容。プラグインが宣言した項目を置く。
   * 閲覧・編集のどちらでも同じ位置に出し、専用の画面を作らない。
   */
  beforeBody?: ReactNode;
  /** 紙の外、いちばん上に出すもの。一覧へ戻る導線のように紙の内容でないもの */
      ...
      <div className="edit-desktop">
        ...
        {(props.localeDrafts ?? [{
          ...
          return (
            ...
            >
              {/*
                プラグインが宣言した項目は、編集のときも同じ面に出す。
                ここに出さないと、参照では読めるのに直す場所がどこにも無くなる。
                正本の言語の面にだけ出す。属性は翻訳ごとに持つものではない。
              */}
              {isActive &&
